...
//Bug 20304
//This function tries to do a very specific thing: It tries to find a twonky media server
//in the existing device list that has the same IP as the one we are looking for
//The idea is that if such a device exists , it is the device we are looking for but with the correct port
const char *FindPossibleTwonkyServerDescDocURLFromOldURL( Device *device, const char *url ) {
    //Basic check: If no description URL return
    if ( !device->descDocURL )
        return NULL;
    //Check to make sure we are dealing with a twonky device
    if( strstr( device->descDocURL, "TMSDeviceDescription.xml" ) == 0 )
        return NULL;
    //Basic check: Our URL starts with http://
    if( strncmp( url, HTTP_PREFIX, strlen( HTTP_PREFIX ) ) != 0 )
        return NULL;
    //Another basic check: The device's description URL starts with http://
    if( strncmp( device->descDocURL, HTTP_PREFIX, strlen( HTTP_PREFIX ) ) != 0 )
        return NULL;
    
    //Basic tests have passed so start from where the IPs begin and compare up to where the ports start
    const char *oldurl = url+strlen( HTTP_PREFIX );
    const char *newurl = device->descDocURL + strlen( HTTP_PREFIX );
    
    //We continue as long as we haven't reached the port setting ( starts with : ) and we are not at the end of either string
    while( *oldurl && *newurl && *oldurl != ':' && *newurl != ':' ) {
        //If any part is different, the IPs are not matching so return
        if( *oldurl != *newurl )
            return NULL;
        oldurl++;
        newurl++;
    }
    //We have exited the loop and everything has matched so far. Just make sure we have not exited because one of the strings ended
    if( *oldurl != *newurl )
        return NULL;
    
    //Looks like we have a winner!
    return device->descDocURL;
}
