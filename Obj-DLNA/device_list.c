...
DeviceList_RemoveDevice (const char* deviceId)
{
	...
	if (node) {
		...
		talloc_free (devnode);
        dispatch_async(dispatch_get_main_queue(), ^{ CFNotificationCenterPostNotification(CFNotificationCenterGetLocalCenter(), CFSTR("deviceRemovedFromUpnpList"), NULL, NULL, kCFNotificationDeliverImmediately);
        } );
...
DeviceList_RefreshAll (bool remove_all)
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{ CFNotificationCenterPostNotification(CFNotificationCenterGetLocalCenter(), CFSTR("refreshedDevicesInUpnplist"), NULL, NULL, kCFNotificationDeliverImmediately);
    } );
	return rc;
}
	...
	if (devnode) {
		...
		*devnode = (struct _DeviceNode) { }; 
		...
		if (devnode->d == NULL) {
			...
			// matches it.
                        // Also make sure g_ssdp_target is valid before doing strstr on it.
			...
			if (node) {
               ...
               dispatch_async(dispatch_get_main_queue(), ^{ CFNotificationCenterPostNotification(CFNotificationCenterGetLocalCenter(), CFSTR("newDeviceAddedToUpnpList"), NULL, NULL, kCFNotificationDeliverImmediately);
               } );
		      ...
		      void* event, void* cookie)
{
	...
	case UPNP_DISCOVERY_SEARCH_RESULT:
	{
        ...
        //Filter out known UPnP ServiceTypes and DeviceTypes that we know we don't support
        if ( strcmp( e->DeviceType, "urn:schemas-upnp-org:device:WANDevice:1" ) == 0
            || strcmp( e->DeviceType, "urn:schemas-upnp-org:device:WANConnectionDevice:1" ) == 0
            || strcmp( e->DeviceType, "urn:schemas-upnp-org:device:InternetGatewayDevice:1" ) == 0
            || strcmp( e->DeviceType, "urn:schemas-upnp-org:device:LANDevice:1" ) == 0
            || strcmp( e->ServiceType, "urn:schemas-upnp-org:service:WANIPConnection:1") == 0
            || strcmp( e->ServiceType, "urn:schemas-upnp-org:service:Layer3Forwarding:1" ) == 0
            || strcmp( e->ServiceType, "urn:schemas-upnp-org:service:WANCommonInterfaceConfig:1" ) == 0
            || strcmp( e->ServiceType, "urn:schemas-upnp-org:service:LANHostConfigManagement:1" ) == 0
            || strcmp( e->ServiceType, "urn:schemas-upnp-org:service:ConnectionManager:1" ) == 0
            )
            break;
...
/*****************************************************************************
 * DeviceList_Start
 * baseurl parameter can be optionally NULL. If not null a device with that IP
 * will forcibly be added to the device list
 *****************************************************************************/
int
...
const char*
FindPossibleTwonkyServerDescDocURLFromOldURL( Device *device, const char *url );

const IXML_Node*
findDeviceNode (const char* const deviceId, IXML_Document* const descDoc);

//Given the URL to the Description of the DLNA server retrieves the device ID
//and device name
void
GetDeviceIdAndNameFromDescription (char* deviceId, const char* descLocation, char *deviceName )
{
	if (descLocation == NULL) {
			Log_Printf (LOG_ERROR,
                        "NULL description doc. URL device Id=%s",
	...
	if (rc != UPNP_E_SUCCESS) {
        ...
        //Bug 20304:
        //If we are here, the descLocation URL can't be retrieved. But there is hope:
        //It's possible that the harcoded port of 9999 we are using for Cisco's twonky has somewhat changed.
        //So we'll try to see if our DLNA engine has picked up a twonky server that starts with the same IP.
        //This block of code goes through devices that have been detected and looks for that possibility
        ListNode* node;
        for (node = ListHead (&GlobalDeviceList);
             node != 0;
             node = ListNext (&GlobalDeviceList, node)) {
            DeviceNode* const devnode = node->item;
            //The function has been named intentionally this way to emphasize that it is for a specific use case
            const char *newDescLocation = FindPossibleTwonkyServerDescDocURLFromOldURL( devnode->d, descLocation );
            if( !newDescLocation )
                continue;
            //We've found a possible descLocation so try to download and use this one
            rc = UpnpDownloadUrlItem ( newDescLocation, &descDocText, content_type);
            if (rc != UPNP_E_SUCCESS) {
                Log_Printf (LOG_ERROR,
                            "Error obtaining device description from NEW "
                            "url '%s' : %d (%s)", newDescLocation, rc,
                            UpnpGetErrorMessage (rc));
                return;
            }
            break;
	...
	rc = ixmlParseBufferEx ( descDocText, &descDoc);
	if (rc != IXML_SUCCESS) {
		Log_Printf (LOG_ERROR, "GetDeviceIDandName can't parse XML "
	 ...
	 */
	char *udn = XMLUtil_FindFirstElementValue ( descDocNode, "UDN", false, false);
    if( udn )
        strcpy( deviceId, udn );
	char *name = XMLUtil_FindFirstElementValue ( descDocNode, "friendlyName", false, false);
    if( name )
